...
```json
{"filename":"sample.jpg","mime_type":"image/jpeg","size_bytes":2048000,"purpose":"identification"}
```

`purpose` 为 `identification`、`avatar` 或 `feedback`。成功返回：

```json
{"asset_id":"10001","upload_url":"https://storage.example/...","expires_in":600}
```

客户端向 `upload_url` 直接上传后，使用 `asset_id` 创建业务资源。服务端必须重新校验 MIME、大小、图片可读性和 SHA-256。
...
返回任务状态、图片、描述、特征和失败信息。非终态不返回鉴别结论。
...
`conclusion` 可为 `authentic`、`counterfeit`、`uncertain`、`unrecognized`。后两者必须展示明确的不确定提示，不能包装成确定结论。任务未完成时返回 `409 RESULT_NOT_READY`。
...
只返回当前用户任务，按 `submitted_at` 倒序。列表项包含 `id`、`status`、首张图片、药材名称、结论、提交时间和完成时间。

### 6.2 查看我的记录详情

`GET /me/identifications/{id}`，需要登录。

返回任务、全部图片、特征、结果、风险提示和参考标准。权限规则与任务详情接口相同。
